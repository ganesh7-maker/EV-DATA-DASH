...
// Interactive Bottom Navigation Dock Handler (HOME, CHARGE, HEALTH, STATS, SETTINGS)
window.selectDockTab = function(element, tabName) {
    if (!tabName) return;

    // 1. Highlight clicked dock item with active cyan border
    const dockItems = document.querySelectorAll('.nav-dock-item');
    dockItems.forEach(item => item.classList.remove('active-dock'));
    ...
    if (element) {
        ...
        const matchingItem = Array.from(dockItems).find(el => el.textContent.includes(tabName));
        if (matchingItem) matchingItem.classList.add('active-dock');
    }

    // 2. Execute smooth navigation based on dock selection
    switch(tabName.toUpperCase()) {
        case 'HOME':
            const simulatorTab = document.querySelector('[data-tab="tab-simulator"]');
            if (simulatorTab) simulatorTab.click();
            window.scrollTo({ top: 400, behavior: 'smooth' });
            break;
        case 'CHARGE':
            const chargingTab = document.querySelector('[data-tab="tab-charging"]');
            if (chargingTab) chargingTab.click();
            break;
        case 'HEALTH':
            const batteryTab = document.querySelector('[data-tab="tab-battery"]');
            if (batteryTab) batteryTab.click();
            break;
        case 'STATS':
            const executiveTab = document.querySelector('[data-tab="tab-executive"]');
            if (executiveTab) executiveTab.click();
            break;
        case 'SETTINGS':
            const govTab = document.querySelector('[data-tab="tab-governance"]');
            if (govTab) govTab.click();
            break;
        default:
            console.log(`Dock tab triggered: ${tabName}`);
    }
};

// One-Click Fleet Telemetry CSV Exporter
window.exportFleetDataCSV = function() {
    const csvHeader = "Session_ID,Fleet_Name,City_Hub,Vehicle_Model,Battery_SOH_Pct,State_of_Charge_Pct,Energy_Consumed_kWh,Charging_Type,Charging_Cost_INR,Timestamp,DTC_Code\n";
    let csvRows = "";
    
    const fleets = ["Premium Fleet", "Green Mobility", "Express Logistics", "EcoPulse"];
    const cities = ["Bengaluru", "Mangalore", "Belagavi", "Mysuru", "Hubballi"];
    const models = ["Tata Nexon EV", "Kia EV6", "Mahindra XUV400", "MG ZS EV", "Hyundai Ioniq 5", "BYD Atto 3"];
    const types = ["Fast Charge", "Normal Charge", "Ultra Fast DC"];
    
    for (let i = 1; i <= 50; i++) {
        const id = `SES-2026-${1000 + i}`;
        const fleet = fleets[i % fleets.length];
        const city = cities[i % cities.length];
        const model = models[i % models.length];
        const soh = (85 + (i * 3) % 15).toFixed(1);
        const soc = (40 + (i * 7) % 55).toFixed(0);
        const energy = (18 + (i * 2.4) % 45).toFixed(1);
        const type = types[i % types.length];
        const cost = (energy * 10).toFixed(2);
        const time = new Date(Date.now() - i * 3600000).toISOString();
        const dtc = i % 15 === 0 ? "P0A7F" : (i % 22 === 0 ? "P0A80" : "NONE");
        
        csvRows += `${id},${fleet},${city},${model},${soh},${soc},${energy},${type},${cost},${time},${dtc}\n`;
    }
    ...
    link.setAttribute("download", `EV_Fleet_Battery_Telemetry_${new Date().toISOString().slice(0,10)}.csv`);
